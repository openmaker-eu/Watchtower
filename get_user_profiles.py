...
def get_all_user_profiles_by_topic(topicID):
    ...
    try:
        userID_list_unprocessed_for_current_topic = Connection.Instance().audienceDB[str(topicID)].distinct('id',{'processed':False})
    except:
    ...
    try:
        for processedProfile in Connection.Instance().audienceDB[str(topicID)].find({'id':{'$in':Connection.Instance().audienceDB['all_audience'].distinct('id')}, 'processed':False}, {'id':1}):
        ...
        #{'$group': ...},
        {'$match': {'in_all_audience':{'$ne':[]}}}, # if the user is in all_audience, this list will include his user id (otherwise, it will be empty)
        {'$project': {'id':1}}
        ]
        ...
        for processedProfile in cursor:
            processed_userID_list.append(processedProfile['id'])
        print("Insertion complete in " + str(time.time()-start) + " seconds. Already processed users list length:" + str(len(processed_userID_list)))
    ...
    unprocessed_userID_list = [userID for userID in userID_list_unprocessed_for_current_topic if userID not in processed_userID_list]
    ...
    processedProfiles = list(Connection.Instance().audienceDB['all_audience'].find({'id':{'$in':processed_userID_list}},{'id':1,'location':1}))
    for processedProfile in processedProfiles:
        ...
        if ( len(operations) == 1000 or (batch_count==int(np.ceil(len(processedProfiles)/1000))-1 and len(operations)==len(processedProfiles)-batch_count*1000)):
            batch_count +=1
            Connection.Instance().audienceDB[str(topicID)].bulk_write(operations,ordered=False)
            operations = []
