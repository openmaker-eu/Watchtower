...
def linkParser(link):
    try:
        ...
        if image != "" and description != "" and title != "":
            dic = {'url': link, 'im': image, 'title': title, 'description': description, 'keywords': keywords,
                   'source': source}
            return dic
...
def calculateLinks(alertid):
    ...
    for tweet in b:
        try:
            ...
            Connection.Instance().db[str(alertid)].find_one_and_update(
                {'id_str': tweet['id_str'], 'isClicked': {'$exist': True}, 'isClicked': False},
                {'$set': {'isClicked': True}})
            ...
            for link in tweet['entities']['urls']:
                ...
                if search('ebay', link) or search('twitter', link):
                    continue
                try:
                    ...
                    if dic != None:
                        if len(list(Connection.Instance().newsPoolDB[str(alertid)].find(
                                {'source': dic['source'], 'title': dic['title']}))) != 0:
                            Connection.Instance().newsPoolDB[str(alertid)].find_one_and_update(
                                {'source': dic['source'], 'title': dic['title']}, {'$push': {'mentions': tweet_tuple}})
